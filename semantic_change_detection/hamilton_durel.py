...
def intersection_align_gensim(m1, m2, words=None):
   ...
   # common_vocab = set([w for w in common_vocab if "pseudoword" in w])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-u", "--cosine_unaligned", action="store_true", default=False,
                        help="Include this flag to rank by cosine dist measure, WITHOUT aligning first")
    ...
    if not options.cosine and not options.neighborhood and not options.cosine_unaligned:
        ...
        if options.cosine_unaligned:
            n_best_by_cosine = rank_by_cosine_unaligned(model1, model2, options.model1_filepath,
                                                        options.model2_filepath, n=options.t_best,
                                                        f=options.frequency_threshold)
        ...
        if options.cosine:
            n_best_by_cosine = rank_by_cosine(model1, model2, options.model1_filepath, options.model2_filepath,
                                              n=options.t_best, f=options.frequency_threshold)
