...
DIR=/home/stud/bernstetter/ma/mainrepo/archiv/synth_eval/hamilton/
...
do
	python $SCRIPT --results_dir=$DIR --results_fn=$(basename $file) --pseudoword_design_dict=$TW_PSEUDO --word_column=1
done

for file in $DIR/dta*.tsv
do
	python $SCRIPT --results_dir=$DIR --results_fn=$(basename $file) --pseudoword_design_dict=$DTA_PSEUDO --word_column=1
done
